...
import type { Consultation } from "@/app/types/types";
...
export default function Grading({ gradings }: ExerciceType) {
	...
	const addExercise = (name: string) => {
		...
		setExo((prev) => {
			const nextIndex =
				prev.length === 0 ? 0 : Math.max(...prev.map((e) => e.index)) + 1;
			...
			prev.map((exercise) => {
				...
				const nextQuestionIndex =
					exercise.questions.length === 0
						? 0
						: Math.max(...exercise.questions.map((q) => q.question_index)) + 1;
	...
	return (
		<div className="space-y-6 p-6">
			...
			{exo?.map((exercise) => (
				<div key={exercise.index} className="border rounded-lg p-4 space-y-4">
					...
					<Input
						...
						onKeyDown={(e) => {
							...
							addQuestion(exercise.index, e.currentTarget.value);
								...
								{isEditing ? (
									...
									<>
										...
										<TrashIcon
											className="cursor-pointer"
											onClick={() =>
												deleteQuestion(exercise.index, question.question_index)
											}
										/>
									</>
								)}
